function [sample] = sample_along_trajectory_timeseries(file, layer, samp)
    ...
    for fld = 1:length(fields)
        ...
        %sample.profiles=F(traj.x,:,traj.t);
...
%             for ix=1:length(traj.x)
%                 sample.profiles=sample.(fields{fld})(ix,:);
%             end
